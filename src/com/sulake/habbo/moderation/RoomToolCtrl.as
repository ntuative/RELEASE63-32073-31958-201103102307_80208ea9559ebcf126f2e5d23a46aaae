package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      public function show() : void
      {
         ...
         this.var_34.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_334));
         Logger.log("BEGINNING TO SHOW: " + this.var_334);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_334);
         if(param1.flatId != this.var_334)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_334);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_34.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_34,WindowTracker.const_809,this._data.flatId),this._frame,false,false,true);
      }
      ...
      private function act(param1:Boolean) : void
      {
         ...
         this.var_34.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1405,_loc2_,this.var_515.text,"",""));
         if(this.var_1418.isSelected || this.var_1417.isSelected || this.var_925.isSelected)
         {
            this.var_34.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1418.isSelected,this.var_1417.isSelected,this.var_925.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1479);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1578);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_34.initMsg.roomMessageTemplates[this.var_1159.selection];
         if(_loc3_ != null)
         {
            this.var_620 = false;
