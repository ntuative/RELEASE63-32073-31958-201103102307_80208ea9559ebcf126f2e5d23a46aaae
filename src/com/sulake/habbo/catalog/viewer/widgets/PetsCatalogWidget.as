package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_502 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_502 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2442 = _loc3_.localizationId;
         this.var_357 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_502,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_502,_loc3_);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_483 < 0 || this.var_483 > this.var_357.length)
         {
            this.var_483 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2442)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_502 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_196.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_502,_loc5_),this.getRaceLocalizationKey(this.var_502,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_196 == null || this.var_736 >= this.var_196.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_483 >= 0 && this.var_483 < this.var_357.length)
         {
            _loc7_ = this.var_357[this.var_483] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_196[this.var_736] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_502,_loc3_,_loc2_,AvatarScaleType.const_57,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_326,AvatarAction.const_802);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_483 >= this.var_357.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_357[this.var_483] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_196[this.var_736] as int;
