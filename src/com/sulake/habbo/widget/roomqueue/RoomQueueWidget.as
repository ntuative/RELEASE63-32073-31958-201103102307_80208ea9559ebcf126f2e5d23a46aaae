package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_354,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_354,this.onQueueStatus);
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1946 = param1.type;
            this.var_1498 = false;
            this.var_1237 = param1.position;
         }
         else
         {
            this.var_1498 = true;
         }
         this.var_2735 = param1.hasHabboClub;
         this.var_1947 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1237.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1237.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1237.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1237.toString());
         this.showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(this.var_1946)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_354:
                  _loc1_.caption = !!this.var_1947 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1498;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_529:
                  _loc1_.caption = !!this.var_1947 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1498;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2735;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_756);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_864));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1946 == RoomWidgetRoomQueueUpdateEvent.const_354)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_852);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
