package com.sulake.habbo.inventory.items
{
   public class FloorItem implements IItem
   {
       
      
      protected var _id:int;
      
      protected var _ref:int;
      
      protected var _category:int;
      
      protected var _type:int;
      
      protected var var_2004:String;
      
      protected var var_1419:Number;
      
      protected var var_2768:Boolean;
      
      protected var var_2769:Boolean;
      
      protected var var_2293:Boolean;
      
      protected var var_2529:Boolean;
      
      protected var var_2767:int;
      
      protected var var_2294:int;
      
      protected var var_2292:int;
      
      protected var var_2295:int;
      
      protected var var_1943:String;
      
      protected var var_2018:int;
      
      protected var var_850:Boolean;
      
      public function FloorItem(param1:int, param2:int, param3:int, param4:int, param5:Boolean, param6:Boolean, param7:Boolean, param8:Boolean, param9:String, param10:Number, param11:int, param12:int, param13:int, param14:int, param15:String, param16:int)
      {
         super();
         this._id = param1;
         this._type = param2;
         this._ref = param3;
         this._category = param4;
         this.var_2293 = param5;
         this.var_2769 = param6;
         this.var_2768 = param7;
         this.var_2529 = param8;
         this.var_2004 = param9;
         this.var_1419 = param10;
         this.var_2767 = param11;
         this.var_2294 = param12;
         this.var_2292 = param13;
         this.var_2295 = param14;
         this.var_1943 = param15;
         this.var_2018 = param16;
      }
      
      public function get id() : int
      {
         return this._id;
      }
      
      public function get ref() : int
      {
         return this._ref;
      }
      
      public function get category() : int
      {
         return this._category;
      }
      
      public function get type() : int
      {
         return this._type;
      }
      
      public function get stuffData() : String
      {
         return this.var_2004;
      }
      
      public function get extra() : Number
      {
         return this.var_1419;
      }
      
      public function get recyclable() : Boolean
      {
         return this.var_2768;
      }
      
      public function get tradeable() : Boolean
      {
         return this.var_2769;
      }
      
      public function get groupable() : Boolean
      {
         return this.var_2293;
      }
      
      public function get sellable() : Boolean
      {
         return this.var_2529;
      }
      
      public function get expires() : int
      {
         return this.var_2767;
      }
      
      public function get creationDay() : int
      {
         return this.var_2294;
      }
      
      public function get creationMonth() : int
      {
         return this.var_2292;
      }
      
      public function get creationYear() : int
      {
         return this.var_2295;
      }
      
      public function get slotId() : String
      {
         return this.var_1943;
      }
      
      public function get songId() : int
      {
         return this.var_2018;
      }
      
      public function set locked(param1:Boolean) : void
      {
         this.var_850 = param1;
      }
      
      public function get locked() : Boolean
      {
         return this.var_850;
      }
   }
}
