package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private function notifyListeners() : void
      {
         ...
         {
            _loc1_ = this.var_522.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.const_1361.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1470.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_642.getValue(_loc6_) == null)
               {
                  ...
                  this.var_642.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_522.indexOf(param3) == -1)
                  {
                     this.var_522.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_642.getKeys())
         {
            _loc5_ = this.var_642.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
