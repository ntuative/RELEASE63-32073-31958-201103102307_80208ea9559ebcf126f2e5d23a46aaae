package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2007:int = 100;
      
      private const const_1411:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_982 = new Timer(this.const_2007,1);
         this.var_982.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_436)
         {
            this.var_436.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_436)
         {
            this.var_436.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_980);
            if(this.var_1472.indexOf("?") > 0)
            {
               _loc2_ = this.var_1472 + "&retry=" + this.var_980;
            }
            else
            {
               _loc2_ = this.var_1472 + "?retry=" + this.var_980;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1903 = true;
         for each(_loc2_ in this.var_1219)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2702,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1220[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1903)
         {
            this.var_1219.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_522[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_522[_loc3_] = _loc6_;
            }
            this.var_981[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_981)
         {
            _loc7_ = true;
            _loc4_ = this.var_981[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_522[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_981[_loc3_];
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_787.length > 0 && this.var_788.length < this.const_1411)
         {
            _loc1_ = this.var_787[0];
            _loc1_.startDownloading();
            this.var_788.push(this.var_787.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_787.indexOf(param1) == -1 && this.var_788.indexOf(param1) == -1)
         {
