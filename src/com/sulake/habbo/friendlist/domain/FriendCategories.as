package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_400:IFriendCategoriesDeps;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            {
               this.method_3(removedFriendId,true);
            }
            status = 3;
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_400.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_400.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_400.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_400.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_400.view.setNewMessageArrived();
                  this.var_400.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
               wasSelected = this.method_3(updatedFriend.id,true);
            ...
            for each(addedFriend in p.addedFriends)
            {
               ...
               this.method_3(addedFriend.id,true);
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_1137).received = true;
      ...
      private function method_3(param1:int, param2:Boolean) : Boolean
      {
         var _loc4_:* = null;
         var _loc5_:* = null;
         if(param2)
