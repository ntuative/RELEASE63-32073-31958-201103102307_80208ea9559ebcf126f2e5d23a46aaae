package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1825:String = "_INCLUDE";
      
      public static const const_1807:String = "_TEMP";
      
      private static const const_1336:String = "layout";
      
      private static const const_957:String = "window";
      
      private static const const_958:String = "variables";
      
      private static const const_959:String = "filters";
      
      private static const const_602:String = "name";
      
      private static const const_1326:String = "style";
      
      private static const const_604:String = "params";
      ...
      private static const const_1327:String = "visible";
      
      private static const const_1337:String = "caption";
      ...
      private static const const_1331:String = "clipping";
      ...
      private static const const_1332:String = "width_min";
      
      private static const WIDTH_MAX:String = "width_max";
      
      private static const const_1338:String = "height_min";
      ...
      private static const const_1330:RegExp = /(\s|\n|\r|\t|\v)*$/;
       
      
      protected var var_1258:Dictionary;
      
      protected var var_1257:Dictionary;
      
      protected var var_1010:Dictionary;
      
      protected var var_1512:Dictionary;
      
      protected var var_2019:Map;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(this.var_1258,this.var_1257);
         this.var_1010 = new Dictionary();
         this.var_1512 = new Dictionary();
         ParamCodeTable.fillTables(this.var_1010,this.var_1512);
         this.var_2019 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1330,const_49);
         return param1.replace(const_1328,const_49);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            for(_loc1_ in this.var_1258)
            {
               delete this.var_1258[_loc1_];
            }
            for(_loc1_ in this.var_1257)
            {
               delete this.var_1257[_loc1_];
            }
            for(_loc1_ in this.var_1010)
            {
               delete this.var_1010[_loc1_];
            }
            for(_loc1_ in this.var_1512)
            {
               delete this.var_1512[_loc1_];
            }
      ...
      public function parseAndConstruct(param1:XML, param2:IWindow, param3:Map) : IWindow
      {
         ...
         var _loc12_:* = null;
         var _loc13_:int = 0;
         if(param1.localName() == const_1336)
         {
            _loc8_ = param1.child(const_958);
            if(_loc8_.length() > 0)
            {
               _loc10_ = _loc8_[0];
            ...
            _loc9_ = param1.child(const_959).children();
            if(_loc9_.length() > 0)
            {
               _loc12_ = new Array();
               _loc13_ = 0;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         var id:int = 0;
         type = this.var_1258[xml.localName()];
         name = unescape(String(this.parseAttribute(xml,const_602,variables,"")));
         style = uint(this.parseAttribute(xml,const_1326,variables,style));
         params = uint(this.parseAttribute(xml,const_604,variables,params));
         ...
         rect.width = Number(this.parseAttribute(xml,const_1262,variables,const_158));
         rect.height = Number(this.parseAttribute(xml,const_1260,variables,const_158));
         visible = this.parseAttribute(xml,const_1327,variables,visible.toString()) == const_603;
         id = int(this.parseAttribute(xml,ID,variables,id.toString()));
         if(xml.child(const_604).length() > 0)
         {
            list = xml.child(const_604).children();
            length = list.length();
            i = 0;
            while(i < length)
            {
               ...
               if(this.var_1010[param] == null)
               {
                  throw new Error("Unknown window parameter \"" + String(node.attribute(const_602)) + "\"!");
               }
               params |= this.var_1010[param];
               i++;
            }
         }
         caption = !!(params & 0) ? (!!parent ? parent.caption : const_49) : const_49;
         caption = unescape(String(this.parseAttribute(xml,const_1337,variables,caption)));
         if(tag != const_49)
         {
            tags = tag.split(const_1261);
            length = tags.length;
            i = 0;
            while(i < length)
            {
         ...
         window = this._context.create(name,null,type,style,params,rect,null,parent is IIterable ? null : parent,id,this.parseProperties(xml.child(const_958)[0]),tags) as WindowController;
         window.limits.minWidth = int(this.parseAttribute(xml,const_1332,variables,window.limits.minWidth));
         window.limits.maxWidth = int(this.parseAttribute(xml,WIDTH_MAX,variables,window.limits.maxWidth));
         window.limits.minHeight = int(this.parseAttribute(xml,const_1338,variables,window.limits.minHeight));
         window.limits.maxHeight = int(this.parseAttribute(xml,const_1329,variables,window.limits.maxHeight));
         background = this.parseAttribute(xml,const_4,variables,window.background.toString()) == const_603;
         blend = Number(this.parseAttribute(xml,const_1228,variables,window.blend.toString()));
         clipping = this.parseAttribute(xml,const_1331,variables,window.clipping.toString()) == const_603;
         ...
         list = xml.child(const_959).children();
         length = list.length();
         if(length > 0)
         {
            filters = new Array();
      ...
      public function windowToXMLString(param1:IWindow) : String
      {
         ...
         var _loc2_:* = const_49;
         var _loc3_:String = this.var_1257[param1.type];
