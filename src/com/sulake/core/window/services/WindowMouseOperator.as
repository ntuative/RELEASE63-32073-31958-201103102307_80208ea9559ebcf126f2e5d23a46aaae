package com.sulake.core.window.services
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_143:DisplayObject;
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            this.var_143.addEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
            this.var_143.addEventListener(MouseEvent.MOUSE_UP,this.handler,false);
            this.var_143.addEventListener(Event.ENTER_FRAME,this.handler);
            this._mouse.x = this.var_143.mouseX;
            this._mouse.y = this.var_143.mouseY;
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            ...
            {
               this.var_143.removeEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
               this.var_143.removeEventListener(MouseEvent.MOUSE_UP,this.handler,false);
               this.var_143.removeEventListener(Event.ENTER_FRAME,this.handler);
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(this._working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               else if(this._mouse.x != this.var_143.mouseX || this._mouse.y != this.var_143.mouseY)
               {
                  this.operate(this.var_143.mouseX,this.var_143.mouseY);
                  this._mouse.x = this.var_143.mouseX;
                  this._mouse.y = this.var_143.mouseY;
               }
               return;
